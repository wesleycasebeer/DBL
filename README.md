...
```
#### Commands
Different commands differ slightly in their syntax and format, but the general format is the command type (norm, count, rand, picture) followed by a period and the command name. Then there will be a set of parentheses, inside of which are the additional bits of information required for a command, with a semicolon at the end, outside of the parentheses.
##### Normal Command
